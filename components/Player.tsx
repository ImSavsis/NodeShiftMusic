...
import Animated, {
  ...
  interpolate, useAnimatedGestureHandler, runOnJS,
  FadeIn, SlideInDown,
...
export default function Player() {
  const { queue, index, playing, progress, duration,
          repeat, shuffle, setExpanded, cycleRepeat, toggleShuffle } = usePlayer();
  const { togglePlay, seek } = useAudio();
  const insets = useSafeAreaInsets();
  const track  = queue[index];

  const translateY = useSharedValue(0);
  const coverScale = useSharedValue(playing ? 1 : 0.88);
  ...
  return (
    <Animated.View style={[StyleSheet.absoluteFill, styles.root, overlayOpacity]} entering={FadeIn.duration(200)}>
      {/* Blurred background */}
      <Image
      ...
      <PanGestureHandler onGestureEvent={gestureHandler}>
        <Animated.View style={[styles.sheet, containerStyle, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 16 }]}>

          {/* Handle + header */}
          <View style={styles.header}>
            <Pressable onPress={close} hitSlop={16} style={styles.handleWrap}>
          ...
          <View style={styles.controls}>
            ...
            >
              {playing ? <PauseIcon size={28} color={Colors.bg} /> : <PlayIcon size={28} color={Colors.bg} />}
            </Pressable>
    ...
    <Animated.View style={style}>
      ...
      >
        <Svg width={24} height={24} viewBox="0 0 24 24" fill="none">
          <Path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" stroke={Colors.text2} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" />
        </Svg>
      </Pressable>
    </Animated.View>
  );
...
function PlayIcon({ size = 22, color = Colors.text }: { size?: number; color?: string }) {
  return <Svg width={size} height={size} viewBox="0 0 24 24" fill={color}><Path d="M8 5v14l11-7z" /></Svg>;
}
function PauseIcon({ size = 22, color = Colors.text }: { size?: number; color?: string }) {
  return <Svg width={size} height={size} viewBox="0 0 24 24" fill={color}><Path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></Svg>;
}
function PrevIcon() {
  return <Svg width={26} height={26} viewBox="0 0 24 24" fill={Colors.text}><Path d="M19 20L9 12l10-8v16z" /><Path d="M5 19V5h2v14H5z" /></Svg>;
}
function NextIcon() {
  return <Svg width={26} height={26} viewBox="0 0 24 24" fill={Colors.text}><Path d="M5 4l10 8-10 8V4z" /><Path d="M19 5v14h-2V5h2z" /></Svg>;
}
...
const styles = StyleSheet.create({
  ...
  artist:      { color: 'rgba(255,255,255,0.55)', fontSize: Font.md, marginTop: 4 },
  progressWrap:{ marginBottom: Spacing.lg },
  timeRow:     { flexDirection: 'row', justifyContent: 'space-between', marginTop: -4 },
  timeText:    { color: 'rgba(255,255,255,0.4)', fontSize: Font.xs },
